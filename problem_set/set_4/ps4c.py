...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    best_pad = []
    best_pad_valid_word_count = 0
    plaintext_message = ""
    for pad in pads:
        unencrypted_text = ciphertext.decrypt_message(pad).get_text()
        unencrypted_text_list = unencrypted_text.split(" ")
        valid_word_count = 0
        for word in unencrypted_text_list:
            if is_word(list_of_words, word):
                valid_word_count += 1
        if valid_word_count >= best_pad_valid_word_count:
            best_pad = pad
            best_pad_valid_word_count = valid_word_count
            plaintext_message = unencrypted_text

    return ps4b.PlaintextMessage(plaintext_message, best_pad)
...
def decode_story():
    ...
    pads = get_story_pads()
    encrypted_message = ps4b.EncryptedMessage(encrypted_story)
    return decrypt_message_try_pads(encrypted_message, pads).get_text()
